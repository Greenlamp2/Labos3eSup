...
package body PACKAGE_ERROR is
   ...
   begin
      ...
      if reraise then
         PACKAGE_ERROR.raise (errcode, errmsg);
      end if;
   end;
   ...
   begin
      if bitand (g_target, c_table) = c_table then
		  ...
		  insert into erreur(dateErreur, errcode, errmessage, namePackage, nameMethod, username) values(sysdate, l_sqlcode, l_sqlerrm, '', '', user);
         commit;
      end if;
   ...
   exception
      when others then
         dbms_output.put_line ('PACKAGE_ERROR::log : INTERNAL ERROR ! ' || SQLERRM);
   end;
   ...
   begin
      if not condition or condition is null then
		 ...
		 log (errmsg => 'Assertion failure: ' || message);
