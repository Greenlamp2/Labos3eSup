--Suppression des tables De la base de Donnée CC1.
...
DROP TABLE CommanderTicket CASCADE CONSTRAINTS;

--Création des tables De la base de Donnée CC1.
create table Erreur(
	idErreur number(6)
...
create table Gestionnaires(
	...
	password varchar2(255)
		constraint NN_GESTIONNAIRES_pass check(password is not null)
);
...
create table CommanderTicket(
	idCommandeTicket number(6),
	dateHeureProjection date,
	numeroSalle number(6),
	nbre number(6),
		
	constraints pk_commanderTicket primary key(idCommandeTicket, dateHeureProjection, numeroSalle),
